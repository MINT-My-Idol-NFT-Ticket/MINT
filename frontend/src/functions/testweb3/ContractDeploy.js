...
const myPrivateKey = '0x93bd1191b6feaea6755c4e3cc3eba0823ce51486b3a848d5f0ffa2a8ce52f00d'
...
const jbPrivateKey = '0x0aa0ac9c31365d81fa78a65063ce913568e65d9a4d43318b15c305bef81ab616'
...
const chPrivateKey = '0x2268d80094b1dcbfcb3785c0940d06ed14d941efe4a80145aced37037833cb7a'
...
;(async () => {
  ...
  const mintContractTest = new web3.eth.Contract(mintTicketAbi, mintContractAddr)
  const saleContractTest = new web3.eth.Contract(saleTicketAbi, saleContractAddr)
  ...
  tx.data = mintContractTest.methods.setSaleTicket(saleContractAddr).encodeABI()
  // //tx.gas = await web3.eth.getGasPrice();
  tx.gas = 300000

  let signedTx = await web3.eth.accounts.signTransaction(tx, myWalletAccount.privateKey)
  ...
  tx.data = ssafyTokenContract.methods.approve(mintContractAddr, 20).encodeABI()
  // //tx.gas = await web3.eth.getGasPrice();
  tx.gas = 3000000

  signedTx = await web3.eth.accounts.signTransaction(tx, jbWalletAccount.privateKey)

  var receipt = await web3.eth.sendSignedTransaction(signedTx.rawTransaction)
  //console.log(receipt);

  /* 정빈 - 구매 전 잔액 */
  const jbsenderBalance = await mintContractTest.methods.getCurrencyAmount().call({ from: jbWalletAddress })
  ...
  /* 정빈 - 보유 티켓 조회 */
  const jbgetTickets = await mintContractTest.methods.getTicketList(jbWalletAccount.address).call()

  console.log('jb의 tokenId: ', jbgetTickets[0].tokenId)
  const jbgetURI = await mintContractTest.methods.tokenURI(jbgetTickets[0].tokenId).call()
  console.log('jb의 tokenURI: ', jbgetURI)

  const jbafterSenderBalance = await mintContractTest.methods.getCurrencyAmount().call({ from: jbWalletAddress })
  ...
  signedTx = await web3.eth.accounts.signTransaction(tx, chWalletAccount.privateKey)

  var receipt = await web3.eth.sendSignedTransaction(signedTx.rawTransaction)
  //console.log(receipt);

  /* 창현 - 구매 전 잔액 */
  const chsenderBalance = await mintContractTest.methods.getCurrencyAmount().call({ from: chWalletAddress })
  ...
  /* 창현 - 보유 티켓 조회 */
  const chgetTickets = await mintContractTest.methods.getTicketList(chWalletAccount.address).call()

  console.log('ch의 tokenId: ', chgetTickets[0].tokenId)
  const getURI = await mintContractTest.methods.tokenURI(chgetTickets[0].tokenId).call()
  console.log('ch의 tokenURI: ', getURI)

  /* 창현 - 보유 잔액 조회 */
  const chafterSenderBalance = await mintContractTest.methods.getCurrencyAmount().call({ from: chWalletAddress })
  console.log('after purchase ticket, ch balance : ', chafterSenderBalance)

  /* mint contract 보유 잔액 */
  const senderBalance = await mintContractTest.methods.getCurrencyAmount().call({ from: mintContractAddr })
  ...
  tx.data = saleContractTest.methods.setForSaleTicket(chgetTickets[0].tokenId, 21).encodeABI()
  // //tx.gas = await web3.eth.getGasPrice();
  tx.gas = 300000
  ...
  /* 등록한 토큰의 가격 */
  const setTicketPrice = await saleContractTest.methods.getTicketPrice(chgetTickets[0].tokenId).call()
  ...
  tx.from = jbWalletAddress
  tx.to = ssafyTokenAddr
  tx.data = ssafyTokenContract.methods.approve(saleContractAddr, setTicketPrice).encodeABI()
  // //tx.gas = await web3.eth.getGasPrice();
  tx.gas = 3000000
  ...
  tx.to = saleContractAddr
  tx.data = saleContractTest.methods.purchaseTicket(chgetTickets[0].tokenId).encodeABI()
  // //tx.gas = await web3.eth.getGasPrice();
  tx.gas = 300000
  ...
  /* 정빈 - 구매한 티켓 조회 */
  const purchaseTicket = await mintContractTest.methods.getTicketList(jbWalletAccount.address).call()
  ...
  tx.from = jbWalletAccount.address
  tx.to = mintContractAddr
  tx.data = mintContractTest.methods.cancelTicket(purchaseTicket[0].tokenId).encodeABI()
  // //tx.gas = await web3.eth.getGasPrice();
  tx.gas = 300000
  ...
  /* 정빈 - 삭제 후 티켓 조회 */
  const afterCancelTicket = await mintContractTest.methods.getTicketList(jbWalletAccount.address).call()

  console.log(afterCancelTicket)

  /* 관리자 - 보유 잔액 조회 */
  const beforeWithdrawBalance = await mintContractTest.methods.getCurrencyAmount().call({ from: myWalletAddress })
  ...
  signedTx = await web3.eth.accounts.signTransaction(tx, myWalletAccount.privateKey)

  var receipt = await web3.eth.sendSignedTransaction(signedTx.rawTransaction)
  // console.log(receipt);

  /* 관리자 - 보유 잔액 조회 */
  const afterWithdrawBalance = await mintContractTest.methods.getCurrencyAmount().call({ from: myWalletAddress })
